...
class PricingController
{
    ...
    // GET /pricing-rules?court_id=X
    public function index(): void
    {
        $court_id = (int)($_GET['court_id'] ?? 0);
        if (!$court_id) { http_response_code(400); echo json_encode(['message' => 'court_id required']); return; }
        $stmt = $this->db->prepare("SELECT * FROM pricing_rules WHERE court_id=? ORDER BY priority DESC, id");
        $stmt->execute([$court_id]);
        echo json_encode(['rules' => $stmt->fetchAll(PDO::FETCH_ASSOC)]);
    }

    // POST /pricing-rules  { court_id, owner_id, name, day_type, start_hour, end_hour, price, valid_from?, valid_to? }
    public function create(): void
    {
        ...
        $ins = $this->db->prepare("
            INSERT INTO pricing_rules (court_id, name, day_type, start_hour, end_hour, price, valid_from, valid_to, priority)
            VALUES (?,?,?,?,?,?,?,?,?)
        ");
        $ins->execute([
            $court_id,
