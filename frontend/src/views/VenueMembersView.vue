    ...
    <div class="min-h-full bg-white">
        ...
        <div class="bg-white px-5 pt-5 pb-4 border-b border-gray-100">
            <div class="flex items-center justify-between mb-3">
                <div>
                    <h1 class="text-lg font-bold text-black">All Members</h1>
                    <p class="text-xs text-gray-500">Active subscribers across all spaces</p>
                </div>
            ...
            <div class="relative">
                ...
                <input v-model="search" type="text" placeholder="Search by name, plan or phone…"
                    class="w-full bg-white border border-gray-200 rounded-xl pl-8 pr-3 py-2.5 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:border border-gray-200" />
            </div>
        </div>
        ...
        <div class="px-4 py-4 pb-8">
            ...
            <div v-else-if="!filtered.length" class="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p class="font-semibold text-gray-700">{{ search ? 'No matching members' : 'No active members' }}</p>
                <p class="text-xs text-gray-400 mt-1">{{ search ? 'Try a different search' : 'Members who subscribe to space plans will appear here' }}</p>
            </div>
