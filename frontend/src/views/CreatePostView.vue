...
const visibilityOptions = [
    { value: 'public',   label: 'Public',   desc: 'Everyone can see this',         icon: Globe,  cls: 'text-black' },
    { value: 'only_me',  label: 'Only Me',  desc: 'Only you can see this',         icon: Lock,   cls: 'text-gray-500' },
...
<template>
    <div class="min-h-full bg-white pb-[calc(11rem+env(safe-area-inset-bottom,0px))]">

        <!-- Booking context badge -->
        <div v-if="courtName" class="px-4 pt-4">
            <div class="bg-gray-50 border border-black/15 rounded-2xl px-4 py-3 flex items-center gap-3">
                <div class="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center shrink-0">
                    <MapPin :size="16" class="text-black" />
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-black text-black leading-none">Sharing about</p>
                    <p class="text-sm font-bold text-black truncate mt-0.5">{{ courtName }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 pt-4">
            <div class="bg-white rounded-3xl shadow-soft ring-1 ring-slate-100 overflow-visible p-4">
                <div class="flex gap-3 items-start">
                    <!-- Avatar -->
                    <div class="w-10 h-10 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center shrink-0 ring-2 border border-gray-200">
                    ...
                    <div class="flex-1 min-w-0 relative">
                        <p class="text-sm font-black text-black leading-none mb-0.5">{{ auth.user?.name }}</p>
                        <textarea
                            ref="textareaRef"
                            :value="postText"
                            ...
                            rows="4"
                            class="w-full text-sm bg-transparent border-none focus:ring-0 resize-none placeholder:text-gray-300 text-black mt-2"
                        />
                        ...
                        <div v-if="showSuggestions"
                            ...
                            <div v-if="mentionLoading" class="flex items-center gap-2 px-4 py-3 text-xs text-gray-400">
                                <Loader2 :size="12" class="animate-spin" /> Searching…
                            </div>
                            <div v-else-if="suggestions.length === 0 && mentionQuery.length >= 1"
                                class="px-4 py-3 text-xs text-gray-400">No results</div>
                            <button v-for="item in suggestions" :key="item.kind + item.id"
                                @mousedown.prevent="selectMention(item)"
                                class="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white transition-colors text-left border-b border-slate-50 last:border-0">
                                <div class="w-9 h-9 rounded-full overflow-hidden shrink-0 bg-gray-100 flex items-center justify-center">
                                    <img v-if="item.avatar_url || item.image_url" :src="item.avatar_url || item.image_url" class="w-full h-full object-cover" />
                                    <span v-else class="text-xs font-extrabold text-black">
                                        {{ item.kind === 'user' ? item.name[0].toUpperCase() : '🏟' }}
                                    </span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-bold text-black truncate">
                                        <span class="text-black">{{ triggerChar }}</span>{{ item.name }}
                                    </p>
                        ...
                        <div v-if="uploadingImages" class="flex items-center gap-2 mt-2 text-[10px] font-bold text-black uppercase tracking-wider">
                ...
                <div v-if="postTags.length" class="flex flex-wrap gap-1.5 mt-3 pl-13">
                    <span v-for="(tag, i) in postTags" :key="i"
                        ...
                        :class="tag.kind === 'user' ? 'bg-gray-50 text-black' : 'bg-violet-50 text-violet-600'">
                ...
                <div class="flex items-center gap-2 mt-4 pt-3 border-t border-slate-50">
                    <label class="cursor-pointer flex items-center gap-1.5 text-[11px] font-bold text-gray-500 hover:text-black px-3 py-2 rounded-xl hover:bg-gray-50 transition-all active:scale-95"
                        :class="selectedFiles.length >= MAX_IMAGES ? 'opacity-30 pointer-events-none' : ''">
                        <ImagePlus :size="16" class="text-black" />
                        <span>Photo</span>
                        <span v-if="selectedFiles.length" class="inline-flex items-center justify-center bg-black text-white text-[9px] font-extrabold w-4 h-4 rounded-full">
                            {{ selectedFiles.length }}
                        </span>
                        <input type="file" accept="image/*" multiple class="hidden" @change="handleImageSelect" />
                    </label>
                    <button @click="insertTrigger('@')"
                        class="flex items-center gap-1.5 text-[11px] font-bold text-gray-500 hover:text-violet-600 px-3 py-2 rounded-xl hover:bg-violet-50 transition-all active:scale-95">
        ...
        <!-- Visibility selector -->
        <div class="px-4 mt-4">
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Who can see this?</p>
            <div class="bg-white rounded-2xl ring-1 ring-slate-100 overflow-hidden shadow-soft">
                <button v-for="opt in visibilityOptions" :key="opt.value"
                    ...
                    :class="visibility === opt.value ? 'bg-black/3' : 'hover:bg-white'">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
                        :class="visibility === opt.value ? 'bg-gray-100' : 'bg-gray-100'">
                        <component :is="opt.icon" :size="18" :class="visibility === opt.value ? opt.cls : 'text-gray-400'" />
                    </div>
                    <div class="flex-1 text-left">
                        <p class="text-sm font-bold" :class="visibility === opt.value ? 'text-black' : 'text-gray-500'">{{ opt.label }}</p>
                        <p class="text-[11px] text-gray-400">{{ opt.desc }}</p>
                    </div>
                    <div v-if="visibility === opt.value"
                        class="w-5 h-5 rounded-full bg-black flex items-center justify-center shrink-0">
        ...
        <div class="fixed left-1/2 z-30 w-full max-w-[430px] -translate-x-1/2 px-4 bottom-[calc(4.5rem+env(safe-area-inset-bottom,0px))]">
            <button
                ...
                class="w-full h-14 rounded-2xl bg-black text-white font-black text-base shadow-xl active:scale-[0.97] disabled:opacity-40 transition-all flex items-center justify-center gap-3">
