...
<template>
    ...
    <div class="min-h-full bg-white">
        ...
        <div class="px-5 py-5 space-y-4">
            ...
            <div v-else-if="spaces.length" class="space-y-3">
                <div v-for="sc in spaces" :key="sc.id"
                    ...
                    <div v-if="editingId !== sc.id" class="flex items-center gap-3">
                        <div class="w-20 h-20 shrink-0 bg-gray-100 flex items-center justify-center overflow-hidden">
                            <img v-if="sc.image_url" :src="sc.image_url" class="w-full h-full object-cover" onerror="this.style.display='none'" />
                            <LayoutGrid v-else :size="22" :stroke-width="1.5" class="text-gray-300" />
                        </div>
                        <div class="flex-1 min-w-0 py-3 pr-1">
                            ...
                            <div class="flex items-center gap-1.5 mt-1">
                                <span class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full"
                                    :class="sc.booking_mode === 'shared' ? 'bg-cyan-50 text-cyan-600' : 'bg-gray-100 text-gray-500'">
                        ...
                        <div class="flex items-center gap-1 mr-2 shrink-0">
                            <button @click="router.push(`/my-venues/${courtId}/spaces/${sc.id}`)"
                                class="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-100 active:scale-90 transition"
                                title="Space settings">
                                <Settings2 :size="14" class="text-black" />
                            </button>
                            <button @click="startEdit(sc)"
                                class="w-9 h-9 flex items-center justify-center rounded-xl bg-white active:scale-90 transition">
                    ...
                    <div v-else>
                        ...
                        <div class="relative h-36 bg-gray-100">
                            ...
                            <div v-else class="w-full h-full flex flex-col items-center justify-center gap-2">
                                <Camera :size="24" class="text-gray-300" />
                                <p class="text-xs text-gray-400">Change photo</p>
                            </div>
                        ...
                        <div class="p-4 space-y-3">
                            <p class="text-[11px] font-black text-gray-400 uppercase tracking-wider">Edit Space</p>
                            ...
                            <input v-model="editForm.description" type="text" placeholder="Description (optional)"
                                class="w-full ring-1 ring-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border border-black focus:outline-none bg-white" />
                            <div>
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1.5">Peak hours (rush windows)</p>
                                <select v-model="editForm.peak_access"
                                    class="w-full ring-1 ring-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border border-black focus:outline-none bg-white font-semibold text-gray-700">
                                ...
                                <p class="text-[10px] text-gray-400 mt-1 px-0.5">Uses the venue’s morning &amp; evening peak times from venue settings.</p>
                            </div>
                            <div>
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1.5">Booking Type</p>
                                <div class="flex gap-2">
                                    <button @click="editForm.booking_mode = 'exclusive'; editForm.capacity = 1"
                                        ...
                                        :class="editForm.booking_mode === 'exclusive' ? 'bg-black text-white' : 'bg-gray-100 text-gray-500'">
                                        <Lock :size="12" /> Exclusive
                                    </button>
                                    <button @click="editForm.booking_mode = 'shared'"
                                        class="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold transition-all"
                                        :class="editForm.booking_mode === 'shared' ? 'bg-cyan-500 text-white' : 'bg-gray-100 text-gray-500'">
                                        <Users :size="12" /> Shared
                                    </button>
                                </div>
                            </div>
                            <div v-if="editForm.booking_mode === 'shared'">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1.5">Max Capacity</p>
            ...
            <p v-else-if="!loading" class="text-center text-gray-400 text-sm py-6">No spaces added yet</p>
            ...
            <div v-if="adding" class="bg-white rounded-2xl overflow-hidden shadow-sm ring-1 ring-slate-100">
                ...
                <div class="relative h-40 bg-gray-100">
                    ...
                    <div v-else class="w-full h-full flex flex-col items-center justify-center gap-2">
                        ...
                        <p class="text-xs text-gray-400">Add photo (optional)</p>
                    </div>
                    <label class="absolute inset-0 cursor-pointer">
                    ...
                    <div v-if="uploadLoading" class="absolute inset-0 bg-white/70 flex items-center justify-center">
                        <Loader2 :size="22" class="animate-spin text-black" />
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <button @click="newMode = 'exclusive'; newCapacity = 1"
                                class="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold transition-all"
                                :class="newMode === 'exclusive' ? 'bg-black text-white' : 'bg-gray-100 text-gray-500'">
                                <Lock :size="12" /> Exclusive
                            </button>
                            <button @click="newMode = 'shared'"
                                class="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold transition-all"
                                :class="newMode === 'shared' ? 'bg-cyan-500 text-white' : 'bg-gray-100 text-gray-500'">
                                <Users :size="12" /> Shared
                            </button>
                        </div>
